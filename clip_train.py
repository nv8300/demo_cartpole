...
def trainIters(shared_linear, meta, actor_a, actor_b, critic_a, critic_b, n_iters):
    ...
    optimizerMeta = optim.Adam(list(meta.parameters()), lr=0.0001)
    schedulerA = lr_scheduler.StepLR(optimizerA, step_size=80, gamma=0.001)
    schedulerC = lr_scheduler.StepLR(optimizerC, step_size=80, gamma=0.001)
    schedulerMeta = lr_scheduler.StepLR(optimizerMeta, step_size=300, gamma=0.01)
    loss_record = []
    for iter in range(n_iters):
        ...
        for i in count():
            ...
            # print(env_action, state[3], meta_parameter)
            next_state, reward, done, _ = env.step(env_action)
            record.append([action_a.detach().numpy(), action_b.detach().numpy(), env_action, state[3].detach().numpy(), meta_parameter.detach().numpy()])
            ...
            results.append(torch.sum(torch.stack(rewards)).item())
            result = torch.sum(torch.stack(rewards)).item()
            # print(torch.sum(torch.stack(rewards)).item())
        ...
        optimizerMeta.step()
        schedulerMeta.step()
    torch.save(actor_a, 'model/actor_a.pkl')
    torch.save(actor_b, 'model/actor_b.pkl')
    torch.save(critic_a, 'model/critic_a.pkl')
    torch.save(critic_b, 'model/critic_b.pkl')
    torch.save(meta, 'model/meta.pkl')
    torch.save(shared_linear, 'model/shared_linear.pkl')
    env.close()
    ...
    with open('record_results.csv', 'w') as w_f:
        writer = csv.writer(w_f)
        writer.writerow(['Action_a', 'Action_b', 'Env_action', 'pole_angular_velocity', 'meta_parameter'])
...
if __name__ == '__main__':
    ...
    trainIters(shared_linear, meta, actor_a, actor_b, critic_a, critic_b, n_iters=200)